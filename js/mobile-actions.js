...
// Keep a clear visual cue around the provider's membership area on mobile.
...
if(membershipFrames.length){
  ...
  const positionScrollCues=()=>{
    ...
    scrollCues.forEach(({container,cue})=>{
      ...
      cue.style.top=Math.max(12,Math.min(rect.height-54,(visibleTop+visibleBottom)/2-rect.top-21))+'px';
    });
  };
